...
func TestKeyring_CRUD(t *testing.T) {
	...
	for _, key := range keys {
		if key.KeyID == id {
			require.Equal(t, RootKeyStateActive, RootKeyState(key.State), "new key should be active")
		} else {
			require.Equal(t, RootKeyStateInactive, RootKeyState(key.State), "initial key should be inactive")
		}
	}
}
